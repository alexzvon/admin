  ...
  <div>
    <div class="block full">
      <div class="block-title clearfix">
        <h1><strong>Быстрый фильтр</strong></h1>
        <div class="block-title-control">
          <b-button variant="success" @click="goCreateQuickFilter">Добавить</b-button>
        </div>
        <hr>
      </div>
...
import { getListQuickFilters } from '../../../api/quick_filter';
...
export default {
  ...
  methods: {
    ...
    onRowClick(item, index) {
      this.$router.push('/shop/quick-filter/edit/' + item.id);
    },

    async getQuickFilterTable() {
      this.isBusy = true;

      try {
        const res = await getListQuickFilters({
          type: this.type,
    ...
    goCreateQuickFilter() {
      this.$router.push('/shop/quick-filter/create');
    },
  },
};
